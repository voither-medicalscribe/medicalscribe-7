...
export async function chatWithAzureAI(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    ...
    if (!endpoint || !apiKey) {
        context.error("Azure AI credentials not configured in environment variables (AZURE_AI_ENDPOINT, AZURE_AI_KEY).");
    ...
    try {
        ...
        if (!response.ok) {
            context.error(`Azure AI error: ${response.status}`, data);
        ...
        context.error('Error in Azure AI chat proxy:', error);
